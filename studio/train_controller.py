...
(``napari_app/core/train_model.py`` + ``train_state_manager.py`` — the exact
...
pulled in lazily inside ``napari_app.core.train_model.train_model`` itself,
...
# SAM backbone checkpoint filenames, keyed the same as napari_app's engines.
BACKBONE_FILES = {
...
class TrainedModel:
    ...
    JSON sidecar (``napari_app.core.train_model._save_config_sidecar``)."""
